...
class SystemicTauApp(BaseApp):
    def __init__(self):
        ...
        self.tagline = ctk.CTkLabel(
            self.sidebar_frame,
            text="Paradigm Edition • τₛ + RECD + Capas",
        ...
        self.recd_check = ctk.CTkCheckBox(self.sidebar_frame, text="RECD + Capas (on)", variable=self.recd_var)
    ...
    def _build_clean_setup_view(self):
        ...
        sub = ctk.CTkLabel(self.current_main_view, 
                           text="For panel count data, locations become coupled modules in a spatial system.\nWe compute multi-variate Kendall τₛ, RECD increments (δ ≈ 4.669), and Capas to reveal reorganization.",
        ...
        desc = ctk.CTkLabel(hero, text="Pivot by location (NID) → Full network τₛ (w=13) → Canonical RECD + Capas\nThe correct flow for systemic analysis of count data across space and time.",
    ...
    def _build_clean_results_view(self, stats):
        ...
        # Create or reuse a clean focused figure (2 subplots: τ_s+RECD and acceleration)
        self.results_fig = Figure(figsize=(10, 4.2), dpi=110, facecolor="#1e2937")
        self.ax_tau = self.results_fig.add_subplot(211)
        self.ax_acc = self.results_fig.add_subplot(212)
    ...
    def _populate_clean_paradigm_plots(self, s):
        """Focused, beautiful plots that actually serve the Systemic Tau paradigm."""
        time_index = np.arange(len(s.get("tau_series", [])))
        self.ax_tau.clear()
        self.ax_acc.clear()

        # Top: τ_s + RECD gate overlay (the heart of the paradigm)
        tau_s = s.get("tau_series", [])
        self.ax_tau.plot(time_index, tau_s, color="#14b8a6", linewidth=1.8, label="Systemic τₛ")
        t_star = s.get("t_star", 0)
        self.ax_tau.axvline(x=t_star, color="#f97316", linestyle="--", linewidth=1.5, label=f"t* = {t_star}")

        # RECD / gate if available
        recd = s.get("recd_dtk") or s.get("recd_array")
        if recd is not None and len(recd) == len(time_index):
            self.ax_tau_twin = self.ax_tau.twinx()
            self.ax_tau_twin.plot(time_index, recd, color="#a78bfa", linewidth=1.2, alpha=0.85, label="RECD (gate)")
            self.ax_tau_twin.set_ylabel("RECD", color="#a78bfa")

        self.ax_tau.set_title("Systemic Kendall τₛ(t) with RECD Discretization", color="#e0f2fe", fontsize=10)
        self.ax_tau.legend(loc="upper right", fontsize=8)
        self.ax_tau.set_facecolor("#1e2937")
        self.ax_tau.tick_params(colors="#94a3b8")

        # Bottom: Acceleration + state
        acc = s.get("acceleration", [])
        self.ax_acc.plot(time_index, acc, color="#f97316", linewidth=1.4, label="a_t (momentum)")
        self.ax_acc.axvline(x=t_star, color="#f97316", linestyle="--", alpha=0.6)
        self.ax_acc.set_title("Acceleration a_t — Critical Momentum at Reorganization", color="#e0f2fe", fontsize=10)
        self.ax_acc.set_facecolor("#1e2937")
        self.ax_acc.tick_params(colors="#94a3b8")
        self.ax_acc.legend(fontsize=8)

        self.results_fig.tight_layout(pad=1.5)
    ...
    def _load_file(self, file_path):
        ...
        try:
            ...
            if self.data_mgr.is_panel:
                ...
                self._build_clean_setup_view()
                if hasattr(self, 'setup_note'):
                    self.setup_note.configure(text=f"Panel ready: {len(self.systemic_targets)} modules • w=13 • RECD+Capas • full systemic τₛ")
    ...
    def _show_recommended_systemic_button(self, principal, loc_count):
        ...
        self.recommended_btn = ctk.CTkButton(
            self.top_bar,
            text=f"🚀 RUN RECOMMENDED SYSTEMIC\n{principal} × {loc_count} modules • w=13 • RECD+Capas",
            command=self._run_recommended_systemic_count,
            height=48,
            fg_color="#166534",
            hover_color="#14532d",
        ...
        hero_desc = ctk.CTkLabel(
            self._hero_cta,
            text=f"Panel detected as spatial system. {loc_count} locations = modules.\nWe pivoted on '{principal}'. We will use w=13, full Kendall τ_s across the network, canonical RECD (Feigenbaum δ) and Capas analysis.",
    ...
    def _render_systemic_ready_state(self):
        ...
        try:
            ...
            if hasattr(self, 'lbl_verdict_desc'):
                ...
                self.lbl_verdict_desc.configure(
                    text=f"Data pivoted: {n} locations as coupled modules • Principal = {p}\n"
                         ...
                         "Click the green rocket to reveal structural reorganization, chaotic regimes and Capas."
                )
            ...
            self._update_status(f"Systemic mode: {n} modules • τ_s + RECD + Capas • Ready for analysis")
        except Exception:
            pass
