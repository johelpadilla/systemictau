...
class TauAnalysisResults:
    ...
    # === Nested ordinal RECD / continuous excess³ (optional; nested-recd) ===
...
def run_full_analysis(
    ...
    **kwargs,
    ...
    # === 7. Nested ordinal RECD / continuous excess³ (optional) ===
    if compute_nested_recd or kwargs.get("compute_nested_recd", False):
        try:
            from .nested import compute_nested_recd as _compute_nested
            ...
            nested_stride = kwargs.get("nested_stride", 1)

            recd_out = _compute_nested(
                np.asarray(X),
                tau_s=results.taus_global,
                m=nested_m,
                d=nested_d,
                theta3=nested_theta3,
                window_tau=nested_window,
                stride=nested_stride,
            )
            ex = np.asarray(recd_out.get("excess3", []), dtype=float)
            results.nested_recd_results = {
                **{k: v for k, v in recd_out.items() if k != "S"},
                "mean_excess3": float(np.nanmean(ex)) if ex.size else 0.0,
                "level3_primary": "excess3",
                "level3_secondary": "phi3",
            }
        except ImportError as e:
            results.warnings.append(
                "compute_nested_recd=True requires nested-recd>=0.2.0 "
                f'(pip install "systemictau[nested]"). Detail: {e}'
            )
        except Exception as e:
            results.warnings.append(f"Error computing Nested RECD / excess³: {e}")
